...
class Sectors(Endpoint):
        ...
        try:
            if sector_name == None:
                results = self.query_db(
                    self.config['sectors']['database_name'], f"SELECT * FROM Sectors WHERE date = '{date}';")
                ...
                results = self.query_db(
                    self.config['sectors']['database_name'], f"SELECT sector_id, date, s_p, dji, nasdaq, russell_1000, russell_2000, vix, vix_close, {clean_sector_name} FROM Sectors WHERE date = '{date}';")
                model = self.make_custom_sector_model(
                    results, clean_sector_name)
                return model[0] if len(model) == 1 else model
    ...
    def make_sector_model(self, sql_results: list):
        ...
        for item in sql_results:
            ...
            model.data['date'] = item[1].strftime(
                '%Y-%m-%d') if item[1] else item[1]
