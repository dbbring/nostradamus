...
class EOD(Endpoint):
  ...
  def get(self, api_endpoint:str, transaction_id:int, include_anaylsis=None) -> dict:
    ...
    results = self.query(api_endpoint, f"SELECT * FROM Price_EOD WHERE transaction_id = {transaction_id};")
    if include_anaylsis:
...
class Charting(Endpoint):
  ...
  def get(self, api_endpoint:str, eod_id:int) -> dict:
    results = self.query(api_endpoint, f"SELECT * FROM Chart_Indicators WHERE eod_id = {eod_id};")
...
class Technical(Endpoint):
  ...
  def get(self, api_endpoint:str, eod_id:int) -> dict:
    results = self.query(api_endpoint, f"SELECT * FROM Technical_Indicators WHERE eod_id = {eod_id};")
  ...
  def make_technical_model(self, sql_results: list):
    ...
    for item in sql_results:
      ...
      model.data['pearsons_coeff_close_sp_5_period'] = item[48] 
      model.data['pearsons_coeff_close_sp_15_period'] = item[49]
      model.data['pearsons_coeff_close_sp_30_period'] = item[50]
