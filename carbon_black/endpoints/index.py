...
class Index(Endpoint):
    ...
    def get_all(self):
        ...
        for data_item in self.config['nostradamus']:
            result = self.query(
                data_item['api_endpoint'], "SELECT * FROM Transaction;")
    ...
    def get_all_by_date(self, date):
        ...
        for data_item in self.config['nostradamus']:
            result = self.query(
                data_item['api_endpoint'], f"SELECT * FROM Transaction WHERE date = '{date}';")
            all_results = all_results + self.make_transaction_model(result)
    ...
    def get_by_id(self, api_endpoint: str, transaction_id: int):
        result = self.query(
            api_endpoint, f"SELECT * FROM Transaction WHERE transaction_id = '{transaction_id}';")
    ...
    def get_db_specific_by_date(self, api_endpoint: str, date: str):
        result = self.query(
            api_endpoint, f"SELECT * FROM Transaction WHERE date = '{date}';")
        return self.make_transaction_model(result)
    ...
    def make_transaction_model(self, sql_results: list) -> list:
        ...
        for trans_item in sql_results:
            ...
            model.data['date'] = trans_item[1].strftime(
                '%Y-%m-%d') if trans_item[1] else None
