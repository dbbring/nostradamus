...
class Bloomberg(Scaper):
    ...
    def __init__(self, firefox_instance=None):
        ...
        table_rows = soup.find_all(
            'div', class_='sector-data-table__sector-row')
...
class FinViz(Scaper):
    ...
    def get_tickers(self, upper_threshold: float, lower_threhold: float) -> list:
        ...
        self.table = self.soup.find('table', attrs={
                                    'bgcolor': '#d3d3d3', 'border': '0', 'cellpadding': '3', 'cellspacing': '1', 'width': '100%'})
        if self.table == None:
            return tickers
    ...
    def has_finviz_news(self) -> bool:
        ...
        for index, row in enumerate(table_rows):
            ...
            news_date = unicodedata.normalize(
                'NFKD', news_date).strip()  # Drop Ending Bytes
...
class SEC_Edgar(Scaper):
    ...
    def __init__(self, ticker: str) -> None:
        ...
        self.base_data = self.base.get_data(
            'https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=' + self.ticker + '&type=&dateb=&owner=exclude&start=0&count=100')
    ...
    def load_data(self, inc: int) -> bool:
        self.soup = self.base.get_data('https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=' +
                                       self.ticker + '&type=&dateb=&owner=exclude&start=' + str(inc) + '&count=100')
    ...
    def get_sic_data(self, location_code: str, sic_code: str) -> list:
        inc = 0
        comps = []
        sic_html = self.base.get_data('https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&SIC=' +
                                      sic_code + '&owner=exclude&match=&start=' + str(inc) + '&count=100&hidefilings=0')
    ...
    def make_arr_peer_performance_model(self, ticker: str) -> list:
        ...
        for index, row in data.iterrows():
            try:
                ...
                peer_model.data['percent_change'] = peer_model.to_percent(
                    row.Open, row.Close)
            ...
            if self.is_adr:
                ...
                for index, row in enumerate(rows):
                    ...
                    if form_id == '6-K' or form_id == 'F-1' or form_id == 'F-3' or form_id == 'F-6' or form_id == 'F-3ASR' or form_id == '20-F':
                        ...
                        self.links_s8.append(
                            'https://www.sec.gov' + link['href'])
                    elif form_id == 'S-3' or form_id == 'S-3ASR':
                        link = self.base.get_table_cell(row, 1, False)
                        link = link.find('a', href=True)
                        self.links_s3.append(
                            'https://www.sec.gov' + link['href'])
                        ...
                        link = self.base.get_table_cell(row, 1, False)
                        link = link.find('a', href=True)
                        self.links_425.append(
                            'https://www.sec.gov' + link['href'])

            if not s1_found:
                last_parsed_date = self.base.get_table_cell(
                    rows[(len(rows) - 1)], 3, True)
    ...
    def make_secondary_offering_model(self, link: str) -> SEC_Secondary_Offering:
        ...
        try:
            ...
            issued_shares = self.base.get_table_cell(
                stock_offering_table_rows[3], 2, True)
            issued_shares = unicodedata.normalize('NFKD', issued_shares)
            issued_shares = issued_shares.split(' America')[0].replace(
                ',', '').replace('shares', '').strip()
            ...
            for row in stock_offering_table_rows:
                row_title = self.base.get_table_cell(
                    row, 0, True).split(',')[0].split('(')[0].strip()
                if row_title.lower() == 'common stock':
                    issued_shares = self.base.get_table_cell(row, 2, True).split(
                        ' Amercia')[0].replace(',', '').split(' shares')[0].split('(')[0]

            # if we parsed the wrong row, oh well, null it out
            model.data['additional_shares_issued'] = int(
                issued_shares) if util.isValidInt(issued_shares) else None
    ...
    def make_merger_model(self, link: str) -> SEC_Merger:
        ...
        try:
            ...
            # Get date from info header
            submission_date = data.find('div', text='Filing Date')
            submission_date = submission_date.find_next_sibling(
                'div').get_text()
    ...
    def make_stock_program_model(self, link: str) -> SEC_Employee_Stock:
        ...
        try:
            ...
            for row in stock_offering_table_rows:
                ...
                if row_title.lower() == 'common stock':
                    issued_shares = self.base.get_table_cell(row, 2, True).split(
                        ' America')[0].replace(',', '').split(' shares')[0].split('(')[0]
...
class TDAmeritrade(Scaper):
    ...
    def __init__(self, ticker: str, index=False):
        ...
        self.news = []
        if (index):
            self.soup = self.base.get_data(
                'https://research.tdameritrade.com/grid/public/research/stocks/charts?symbol=' + ticker)
        else:
            self.soup = self.base.get_data(
                'https://research.tdameritrade.com/grid/public/research/stocks/summary?fromPage=overview&display=&fromSearch=true&symbol=' + ticker)
        return
