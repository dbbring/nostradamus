...
try:
    ...
    for data_item in config['nostradamus']:
        ...
        while not end_of_list:
            FV = FinViz((data_item['screener_url'] +
                         str(finviz_pg_index)), browser)
            pg_tickers = FV.get_tickers(
                data_item['upper_bound'], data_item['lower_bound'])
        ...
        for ticker in tickers:
            try:
                FV = FinViz('https://finviz.com/quote.ashx?t=' +
                            ticker, browser)
                if (FV.has_finviz_news()):
                    ...
                    if not TD.has_td_news() and ticker_count < data_item['num_of_tickers_to_process']:
                        ...
                        new_thread = Process(target=process_ticker, args=(
                            data_item['database_name'], TD, FV, ticker, sp_inputs))
                        new_thread.start()
            ...
            for ticker_info in tickers_need_updated:
                try:
                    ...
                    for index in range(start, end, -1):
                        eod_data = symbol.make_price_eod_model(
                            inputs, index, is_tracking=True)
